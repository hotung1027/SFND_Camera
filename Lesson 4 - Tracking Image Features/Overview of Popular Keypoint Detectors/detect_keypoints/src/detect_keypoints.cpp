...
void detKeypoints1() {
  ...
  int threshold = 100;
  cv::FastFeatureDetector::DetectorType FastDetectorType =
      cv::FastFeatureDetector::TYPE_9_16;
  cv::Ptr<cv::FastFeatureDetector> fast =
      cv::FastFeatureDetector::create(threshold, true, FastDetectorType);
  std::vector<cv::KeyPoint> fast_kpts;
  double fastTickCount = (double)cv::getTickCount();
  fast->detect(imgGray, fast_kpts);
  fastTickCount =
      ((double)cv::getTickCount() - fastTickCount) / cv::getTickFrequency();
  cout << "Fast Detector with n= " << fast_kpts.size() << " keypoints in "
       << 1000 * fastTickCount / 1.0 << " ms" << endl;

  cv::Mat visImageFast;
  drawKeypoints(img.clone(), fast_kpts, visImageFast);
  imshow("Fast Keypoints", visImageFast);
  cv::waitKey(0);
  return;
}
