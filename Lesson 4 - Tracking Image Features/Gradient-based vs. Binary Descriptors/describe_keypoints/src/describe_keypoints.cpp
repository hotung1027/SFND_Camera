...
void descKeypoints1() {
  ...
  cout << "BRISK detector with n= " << kptsBRISK.size() << " keypoints in "
       << 1000 * t / 1.0 << " ms" << endl;
  ...
  cv::drawKeypoints(img, kptsBRISK, visImage, cv::Scalar::all(-1),
  ...
  cv::Ptr<cv::SIFT> detectorSIFT =
      cv::SIFT::create(nfeatures, nOctaveLayers, contrastThreshold,
                       edgeThreshold, sigma, enable_precise_upscale);
  // cv::Ptr<cv::SIFTDescriptorExtractor> descriptorSIFT = cv::SIFT::create();
  // cv::SiftDescriptorExtractor;
  vector<cv::KeyPoint> kptsSIFT;
  cv::Mat visImageSIFT = img.clone();
  t = (double)cv::getTickCount();
  detectorSIFT->detect(imgGray, kptsSIFT);
  cv::drawKeypoints(img, kptsSIFT, visImageSIFT, cv::Scalar::all(-1),
                    cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
  t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
